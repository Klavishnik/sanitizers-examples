...
Данный флаг не влияет на безопасноть, но упомянуть о нём стоило.
...
```
При перехвате сигнала SIGABRT (Abort) будет создан core файл, содержащий инфомарцию от процессе.
...
На этапе компиляции неоходимо добавить флаги:

`-g` - флаг, подключающий отладочную информацию. При срабатывании санитайзера, благодаря данному флагу, в отладочной информации отоборазится номер строки исходного кода, на которую сработал санитайзер. Без данного флага вместо номера строки с небезопасной конструкцией будет отображаться её адрес в памяти. 
...
### Релиз - отдельно, тестировние - отдельно

Важно помнить, что включение проверок увеличивает размер скомпилированного файла и замедляет программу из-за выполнения проверок.

Поэтому необходимо делать отдельно **Debug** и **Release** сборки. 

1. Debug-сборка выполняется с включением специльных флагов компиляции / санитайзеров. На ней же выполняются все тесты и проверки.
